...
def rand_flip4(img, label, label1, label2):
    ...
    if r < 0.25:
        ...
        return np.flip(img, axis=0).copy(), np.flip(label, axis=0).copy(), np.flip(label1, axis=0).copy(), np.flip(
            label2, axis=0).copy()
    elif r < 0.75:
        return np.flip(img, axis=1).copy(), np.flip(label, axis=1).copy(), np.flip(label1, axis=1).copy(), np.flip(
            label2, axis=1).copy()
    else:
