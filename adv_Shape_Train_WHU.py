...
#######################################################
from datasets import Building_WHU as RS
...
def main():
    ...
    train_set = RS.RS('train', random_crop=True, crop_nums=args['num_crops'], crop_size=args['train_crop_size'],
                      random_flip=True)  # Mas
    ...
    for curr_epoch in range(args['epochs']):
        ...
        for i_iter, data in enumerate(train_loader):
            ...
            if i_iter % args['print_freq'] == 0:
                print('Iter %d/%d, Seg_loss: %.2f, adv_loss: %.2f, Dpred_loss: %.2f, Dgt_loss: %.2f'
                      % (i_iter, iters_per_epoch, seg_loss_meter.val, adv_loss_meter.val, Dpred_loss_meter.val,
        ...
        if val_F > bestF:
            ...
            bestacc = val_acc
            bestIoU = val_IoU
            torch.save(model_seg.state_dict(), os.path.join(args['chkpt_dir'],
                                                            NET_NAME + '_e%d_OA%.2f_F%.2f_IoU%.2f.pth' % (
                                                                curr_epoch, val_acc * 100, val_F * 100, val_IoU * 100)))
        print('[epoch %d] [lr %f] [Val loss: %.2f Acc %.2f F1 score: %.2f IoU %.2f]' % (
            curr_epoch, optimizer.param_groups[0]['lr'], val_loss, val_acc * 100, val_F * 100, val_IoU * 100))
